...
public class SVNUtil {  
	...
	private static SVNRepository sRepository;
	private static SVNClientManager sClientManager;
    ...
    public static SVNRepository createRepository(String svnRoot, String username,  
            String password) {
    	// 初始化版本库  
        setupLibrary();  
...
//                    .parseURIEncoded(svnRoot));  
        	sRepository = DAVRepositoryFactory.create(SVNURL.parseURIEncoded(svnRoot));
        ...
        // 创建身份验证管理器  
        sRepository.setAuthenticationManager(authManager);  
        
        DefaultSVNOptions options = SVNWCUtil.createDefaultOptions(true);  
        sClientManager = SVNClientManager.newInstance(options, authManager);  
    ...
    public static SVNClientManager authSvn(String svnRoot, String username,  
        ...
        try {  
            repository = SVNRepositoryFactory.create(SVNURL  
                    .parseURIEncoded(svnRoot));  
//        	repository = DAVRepositoryFactory.create(SVNURL.parseURIEncoded(svnRoot));
        } catch (SVNException e) {  
        	Logger.error(e.getErrorMessage(), e);  
            return null;  
        }  
     ...
     * @param revision
     */
	public static void exportSingleFile(String changedPath, String dstPath, long revision) {
		try {
			String path = sRepository.getRepositoryRoot(false).getPath();
			path += "/";
			path += changedPath;
